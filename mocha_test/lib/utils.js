...
module.exports.verifyTransactionHash = function(response) {
  ...
  assert.equal(
    eventListener.verifyIfEventFired(response.data.transaction_uuid, 'transaction_initiated'),
    true,
    'Event verification for transaction_initiated'
  );
};
...
module.exports.verifyTransactionReceipt = function(response) {
  ...
  assert.equal(
    eventListener.verifyIfEventFired(response.data.transaction_uuid, 'transaction_mined'),
    true,
    'Event verification for transaction_mined'
  );
};
