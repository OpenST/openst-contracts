...
const userAirdropDetailCache = module.exports = function (params) {
  logger.info("===========cacheManagement.user_airdrop_details.params===============");
  logger.info(params);
  const oThis = this;
...
userAirdropDetailCache.prototype.fetchDataFromSource = async function (cacheIds) {
  const oThis = this;
  logger.info("===========cacheManagement.user_airdrop_details.fetchDataFromSource.cacheIds===============");
  logger.info(cacheIds);
  if (!cacheIds) {
  ...
  logger.info("===========cacheManagement.user_airdrop_details.callingFromDB===============");
...
userAirdropDetailCache.prototype.getCacheKey = function(address) {
  const oThis = this;
  return `${oThis.chainId}_${oThis.airdropId}_${address}`;
};
