...
require(rootPrefix + '/lib/airdrop_management/adjust_airdrop_amount');
 ...
 * @constructor
 */
const TransactionHelper = function(chainId) {
  const oThis = this;
...
TransactionHelper.prototype = {
  ...
  updateAirdropBalance: function(airdropContractAddress, owner, toCreditBigAmount) {
    ...
    if (basicHelper.convertToBigNumber(toCreditBigAmount).gt(0)) {
      ...
      return oThis.debitAirdropBalance(
  ...
  creditAirdropBalance: async function(airdropContractAddress, owner, bigAmount) {
    const oThis = this,
      AdjustAirdropAmountKlass = oThis.ic().getAdjustAirdropAmountClass();
  ...
  clearUserDetailCache: async function(airdropContractAddress, owner) {
    const oThis = this,
      ...
      airdropModelCacheObject = new AirdropModelCacheKlass({
        useObject: true,
   ...
   *
   */
  afterPaySuccess: async function(
    brandedTokenAddress,
    spenderAddress,
    estimatedPayAmount,
    beneficiaryAddress,
   ...
   *
   */
  beforeAirdropPay: async function(
    brandedTokenAddress,
    airdropAddress,
    spenderAddress,
    totalTransferAmount,
    ...
    logger.debug('lib/transaction_helper.js:beforeAirdropPay called with params:', {
      ...
      spender: spenderAddress,
      estimatedAirdropAmount: airdropBalanceToUse,
      airdropBugdetAddress: airdropBugdetAddress
    });
    ...
    promiseArray.push(
      oThis.updateAirdropBalance(
        airdropAddress,
        spenderAddress,
        airdropBalanceToUse.mul(basicHelper.convertToBigNumber(-1))
      )
    );
   ...
   * @return {promise<result>}
   */
  afterAirdropPaySuccess: async function(
    brandedTokenAddress,
    airdropAddress,
    spenderAddress,
    estimatedPayAmount,
    ...
    logger.debug('lib/transaction_helper.js:afterAirdropPaySuccess called with params:', {
      ...
      estimatedAirdropAmount: estimatedAirdropAmount,
      airdropBugdetAddress: airdropBugdetAddress,
    ...
    promiseArray.push(
      oThis.updateAirdropBalance(airdropAddress, spenderAddress, estimatedAirdropAmount.minus(actualAirdropAmount))
    );
...
InstanceComposer.registerShadowableClass(TransactionHelper, 'getTransactionHelperClass');
