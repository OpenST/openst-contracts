...
TransactionHelper.prototype = {
  ...
  creditAirdropBalance: function (airdropContractAddress, owner, bigAmount) {
    ...
    return oThis.updateAirdropDetail(owner, bigAmount);
  },
   ...
   *
   * @param {string} owner - Account address
   * @param {BigNumber} airdropAmountUsedBigNumber - airdrop amount to be set
   *
   * @return {promise<result>}
   *
   * @ignore
   */
  updateAirdropDetail: async function (owner, airdropAmountUsedBigNumber) {

    const userAirdropDetailModel = new UserAirdropDetailModelKlass();
    return await userAirdropDetailModel.updateAirdropUsedAmount(
                  owner,
                  airdropAmountUsedBigNumber.toString()
                );
  },
