...
TransactionHelper.prototype = {
  ...
  creditAirdropBalance: async function (airdropContractAddress, owner, bigAmount) {
    ...
    var r = await userAirdropDetailModel.creditAirdropUsedAmount(airdropContractAddress, owner, bigAmount.toString(10));
  ...
  debitAirdropBalance: async function (airdropContractAddress, owner, bigAmount) {
    const oThis = this;
    const userAirdropDetailModel = new UserAirdropDetailModelKlass();
    var r = await userAirdropDetailModel.debitAirdropUsedAmount(airdropContractAddress, owner, bigAmount.toString(10));
  ...
  beforeAirdropPay: function(
    ...
    const debitAirdropBudgetHolder = oThis.debitBalance(brandedTokenAddress, airdropBugdetAddress, estimatedAirdropAmount);
    return Promise.all([beforePayResult, debitAirdropBalance, debitAirdropBudgetHolder]);
  },
  ...
  afterAirdropPaySuccess: function(
    ...
    const adjustSpenderAmount = new Promise(function(onResolve, onReject) {
      ...
      if (actualDebitAmount.gt(estimatedDebitAmount)) {
        ...
        oThis.debitBalance(brandedTokenAddress, spender, adjustedAmount).then(function(senderAdjustedDebitCacheResponse) {
          onResolve(senderAdjustedDebitCacheResponse);
        });
      } else if (actualDebitAmount.lt(estimatedDebitAmount)) {
        const adjustedAmount = estimatedDebitAmount.minus(actualDebitAmount);
        oThis.creditBalance(brandedTokenAddress, spender, adjustedAmount).then(function(senderAdjustedCreditCacheResponse) {
          onResolve(senderAdjustedCreditCacheResponse);
        });
      } else {
        onResolve(responseHelper.successWithData({}));
    ...
    const adjustmentAirdropBalancePromise = new Promise(function (onResolve, onReject) {
      ...
      if (actualAirdropAmount.gt(estimatedAirdropAmount)) {
        ...
        oThis.debitAirdropBalance(airdropAddress, spender, adjustedAmount).then(function(senderAdjustedDebitCacheResponse) {
          onResolve(senderAdjustedDebitCacheResponse);
        });
        ...
        oThis.creditAirdropBalance(airdropAddress, spender, adjustedAmount).then(function(senderAdjustedCreditCacheResponse) {
          onResolve(senderAdjustedCreditCacheResponse);
        });
      } else {
        onResolve(responseHelper.successWithData({}));
    ...
    //ugly code here, change this once you revisit, merge the logic with above code
    const adjustmentAirdropBudgetHolderBalancePromise = new Promise(function (onResolve, onReject) {

      if (actualAirdropAmount.gt(estimatedAirdropAmount)) {
        const adjustedAmount = actualAirdropAmount.minus(estimatedAirdropAmount);
        oThis.debitBalance(brandedTokenAddress, airdropBugdetAddress, adjustedAmount).then(function(senderAdjustedDebitCacheResponse) {
          onResolve(senderAdjustedDebitCacheResponse);
        });
      } else if (actualAirdropAmount.lt(estimatedAirdropAmount)) {
        const adjustedAmount = estimatedAirdropAmount.minus(actualAirdropAmount);
        oThis.creditBalance(brandedTokenAddress, airdropBugdetAddress, adjustedAmount).then(function(senderAdjustedCreditCacheResponse) {
          onResolve(senderAdjustedCreditCacheResponse);
        });
      } else {
        onResolve(responseHelper.successWithData({}));
    ...
    const allPromises = [creditBeneficiaryBalance, creditCommissionBeneficiaryBalance, adjustSpenderAmount, adjustmentAirdropBalancePromise, adjustmentAirdropBudgetHolderBalancePromise];
  ...
  afterAirdropPayFailure: function (
    ...
    const creditAirdropBudgetHolder = oThis.creditBalance(brandedTokenAddress, airdropBugdetAddress, estimatedAirdropAmount);
    const allPromises = [creditAirdropBalance, creditAirdropBudgetHolder];
