...
AdjustAirdropAmountKlass.prototype = {
  ...
  debitAirdropUsedAmount: function() {
    ...
    return new Promise(async function(onResolve, onReject) {
      ...
      try {
        ...
        const userAirdropDetailResults = await userAirdropDetailModel
          .select(
            'id, airdrop_id, user_address, CONVERT(airdrop_amount, char) as airdrop_amount, CONVERT(airdrop_used_amount, char) as airdrop_used_amount'
          )
          .where({ airdrop_id: airdropRecord.id, user_address: oThis.userAddress })
          .where(['airdrop_amount > airdrop_used_amount'])
          .fire();
        ...
        for (var uadIndex in userAirdropDetailResults) {
          ...
          const updateResult = await userAirdropDetailModel
            .update(['airdrop_used_amount=airdrop_used_amount+?', amountToAdjustWithCurrentRecord.toString(10)])
            .where([
              'id = ? AND ((airdrop_used_amount+?) <= airdrop_amount)',
              uad.id,
              amountToAdjustWithCurrentRecord.toString(10)
            ])
            .fire();
          ...
          if (updateResult.affectedRows < 1) {
            ...
            logger.debug(
              ...
              '\ntotalRemainingAmountToAdjust: ',
              totalRemainingAmountToAdjust.toString(10)
            );
          }
        }
      } catch (err) {
  ...
  rollbackDebitAirdropAdjustedAmount: async function(amountAdjustedLog) {
    ...
    for (var uadId in amountAdjustedLog) {
      ...
      await userAirdropDetailModel
        .update(['airdrop_used_amount=airdrop_used_amount-?', rollbackAmount])
        .where(['id = ?', uadId])
        .fire();
    }
  },
    ...
    return new Promise(async function(onResolve, onReject) {
      ...
      try {
        ...
        const userAirdropDetailResults = await userAirdropDetailModel
          ...
          .where({ airdrop_id: airdropRecord.id, user_address: oThis.userAddress })
          .where(['airdrop_used_amount > 0 AND airdrop_amount >= airdrop_used_amount'])
          .fire();
        ...
        for (var uadIndex in userAirdropDetailResults) {
          ...
          userAirdropDetailModel = new userAirdropDetailKlass();
          const updateResult = await userAirdropDetailModel
            .update(['airdrop_used_amount=airdrop_used_amount-?', toAdjustAmount.toString(10)])
            .where([
              'id = ? AND (airdrop_amount >= airdrop_used_amount) AND (airdrop_used_amount-?)>=0',
              uad.id,
              toAdjustAmount.toString(10)
            ])
            .fire();
          logger.debug('\ndebitAirdropUsedAmount.updateResult: ', updateResult);
          if (updateResult.affectedRows < 1) {
      ...
      if (totalRemainingAmountToAdjust.gt(0)) {
        ...
        logger.error(
          '%Error - Airdrop used Amount not adjusted. This could be because of same user payment requests in parallel'
        );
        let errorParams = {
...
InstanceComposer.registerShadowableClass(AdjustAirdropAmountKlass, 'getAdjustAirdropAmountClass');
