    ...
    return new Promise(async function (onResolve, onReject) {
      ...
      try {
        ...
        if (totalRemainingAmountToAdjust.lte(0)) {
          return onResolve(responseHelper.error('l_am_aaa_daua_1', 'zero or negative airdropAmountUsed not allowed'));
        }
  ...
  creditAirdropUsedAmount: function () {
    ...
    return new Promise(async function (onResolve, onReject) {
      ...
      try {
        ...
        var totalRemainingAmountToAdjust = new BigNumber(oThis.airdropAmountUsed);
        if (totalRemainingAmountToAdjust.lte(0)) {
          return onResolve(responseHelper.error('l_am_aaa_caua_1', 'zero or negative airdropAmountUsed not allowed'));
        }
        if (!oThis.userAddress) {
