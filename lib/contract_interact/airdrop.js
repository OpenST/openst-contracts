...
Airdrop.prototype.pay = async function (senderWorkerAddress, senderWorkerPassphrase, beneficiaryAddress,
  ...
  const successCallback = async function(receipt) {
    ...
    if (actualAmountFromReceipt.isSuccess()) {
      return await oThis.onAirdropPaySuccess(postPayParams, actualAmountFromReceipt.data);
    } else {
  ...
  if (options.shouldHandlePostPay == 1) {
    params.shouldHandlePostPay = 1;
  } else {
    params.shouldHandlePostPay = 0;
    params.postPayParams = postPayParams;
  }
...
Airdrop.prototype.postAirdropPay = async function(airdropPostPayParams, decodedEvents, status) {
  ...
  if (status == 1) {
    ...
    const actualAmountFromReceipt = oThis.transactionHelper.getActualAmountsFromDecodedEvents(decodedEvent,
      eventGlobalConstants.eventAirdropPayment());

    if (actualAmountFromReceipt.isSuccess()) {
      return await oThis.onAirdropPaySuccess(airdropPostPayParams, actualAmountFromReceipt.data);
    }
  }
 ...
 *
 * @param {Object} airdropPostPayParams - airdrop post pay params
 * @param {Object} actualAmountFromReceipt - Actual transfer amounts from receipt
 *
 * @return {promise<result>}
 *
 */
Airdrop.prototype.onAirdropPaySuccess = async function (airdropPostPayParams, actualAmountFromReceipt) {

  try {
    const validationResponse = helper.validateAfterPayParams(airdropPostPayParams);
    ...
    const brandedTokenAddress = airdropPostPayParams.brandedTokenAddress
      ...
      , totalAmount = airdropPostPayParams.totalAmount
      , airdropAmountToUse = airdropPostPayParams.airdropAmountToUse
    ...
    const afterSuccessResponse = await oThis.transactionHelper.afterAirdropPaySuccess(
      ...
      airdropAmountToUse,
      beneficiaryAddress,
      actualAmountFromReceipt.actualBeneficiaryAmount,
      commissionBeneficiaryAddress,
      actualAmountFromReceipt.actualCommissionAmount,
      actualAmountFromReceipt.actualAirdropAmount,
      airdropBudgetHolder);

    const isAllResponseSuccessful = oThis.transactionHelper.isAllResponseSuccessful(afterSuccessResponse);
    if (isAllResponseSuccessful) {
      return Promise.resolve(afterSuccessResponse);
    }
    return Promise.resolve(responseHelper.error('l_ci_a_onAirdropPaySuccess_1', 'Something went wrong'));
  } catch (err) {
    logger.error("lib/contract_interact/airdrop.js:onAirdropPaySuccess inside catch ", err);
    return Promise.resolve(responseHelper.error('l_ci_a_onAirdropPaySuccess_2', 'Something went wrong'));
  }
...
Airdrop.prototype.onAirdropPayFailure = async function (airdropPostPayParams) {
  ...
  try {
    ...
    if (isAllResponseSuccessful) {
      ...
      return Promise.resolve(responseHelper.error('l_ci_a_onAirdropPayFailure_1', 'Something went wrong'));
    }
  } catch(err) {
    logger.error("lib/contract_interact/airdrop.js:onAirdropPayFailure inside catch ", err);
    return Promise.resolve(responseHelper.error('l_ci_a_onAirdropPayFailure_2', 'Something went wrong'));
  }
