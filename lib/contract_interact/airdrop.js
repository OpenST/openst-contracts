...
require(rootPrefix + '/lib/cache_management/airdrop');
...
const Airdrop = function(airdropContractAddress, chainId) {
  const oThis = this,
    AirdropCacheKlass = oThis.ic().getCacheManagementAirdropClass();
...
const AirdropSpecificPrototype = {
  ...
  /**
   * pay method which is called from outside - can give rejections of Promise
   *
   ...
   * @param {string} airdropBudgetHolder - airdrop budget holder address
   *
  ...
  /**
   * Async pay - this method is internal and is called from the pay method
   */
  _asyncPay: async function(
    ...
    const oThis = this,
      ...
      AirdropUserBalanceKlass = oThis.ic().getAirdropUserBalanceClass(),
      web3Provider = web3ProviderFactory.getProvider(web3ProviderFactory.typeWS),
      contractAbi = coreAddresses.getAbiForContract('airdrop'),
      currContract = new web3Provider.eth.Contract(contractAbi);

    await helper.validateAirdropPayParams(
      senderWorkerAddress,
      beneficiaryAddress,
      transferAmount,
      commissionBeneficiaryAddress,
    ...
    if (!currency) {
      totalTransferAmount = basicHelper
    ...
    let spenderOnChainAvailableBalance = basicHelper.convertToBigNumber(
      spenderOnChainAvailableBalanceResponse.data.balance
    );
    ...
    logger.debug(
      `\nuserInitialBalance.plus(airdropBalanceToUse): ${spenderOnChainAvailableBalance
        .plus(airdropBalanceToUse)
        .toString(10)}`
    );
    ...
    if (!brandedTokenAddress) {
      ...
      oThis.brandedTokenAddress = brandedTokenResponse.data.brandedToken;
    }

    let airdropBudgetHolderAddress = airdropBudgetHolder;

    if (!airdropBudgetHolder) {
      ...
      airdropBudgetHolderAddress = airdropBudgetHolderResponse.data.airdropBudgetHolder;
    }
    ...
    const postPayParams = {
      ...
      airdropBudgetHolder: airdropBudgetHolderAddress,
    ...
    const successCallback = async function(receipt) {
      ...
      const actualAmountsFromReceipt = oThis.transactionHelper.getActualAmountsFromReceipt(
        receipt,
    ...
    if (isAllResponseSuccessful) {
      ...
      logger.error(
        '%Error - Something went wrong. It could be that either cache or airdrop used amount promise failed.'
      );
   ...
   * @param {boolean} status - transactions status (false => failure, true => success)
   *
   * @return {promise<result>}
   *
   */
  postAirdropPay: async function(airdropPostPayParams, decodedEvents, status) {
    ...
    if (status) {
      const actualAmountsFromReceipt = oThis.transactionHelper.getActualAmountsFromDecodedEvents(
        decodedEvents,
        eventGlobalConstants.eventAirdropPayment()
      );

      if (actualAmountsFromReceipt.isSuccess()) {
        return oThis.onAirdropPaySuccess(airdropPostPayParams, actualAmountsFromReceipt.data);
      }
    }

    return oThis.onAirdropPayFailure(airdropPostPayParams);
  },
    ...
    try {
      ...
      if (cacheResult.isSuccess() && cacheResult.data.response != null) {
        ...
        if (getAirdropBudgetHolderFromContractResponse.isSuccess()) {
          await oThis.airdropCache.setAirdropBudgetHolder(
            getAirdropBudgetHolderFromContractResponse.data.airdropBudgetHolder
          );
        }
...
InstanceComposer.registerShadowableClass(Airdrop, 'getAirdropInteractClass');
