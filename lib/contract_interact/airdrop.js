...
Airdrop.prototype.pay = async function (senderWorkerAddress, senderWorkerPassphrase, beneficiaryAddress,
  ...
  // we need to load the balance of beneficiaryAddress and commissionBeneficiaryAddress in the cache before
  // processing pay, reason being after the receipt when the amount is credited to beneficiaryAddress and
  // commissionBeneficiaryAddress, we increment the balance in cache. So if the initial amount is not in cache
  // it gets from the contract and its the actual balance. So they already have the actual balance and
  // crediting again will result in to double amount in cache.

  await oThis.getBalanceOf(beneficiaryAddress);
  if(commissionBeneficiaryAddress && commissionBeneficiaryAddress.length > 0) {
    await oThis.getBalanceOf(commissionBeneficiaryAddress);
  }
