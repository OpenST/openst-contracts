 ...
 * @constructor
 */
const ContractInteractHelper = function(configStrategy, instanceComposer) {};
...
ContractInteractHelper.prototype = {
  ...
  performSend: function(params, returnType) {
    ...
    const onReceiptFail = async function(errorCode, errorReason, receipt) {
      ...
      const errorMessage =
        errorReason && errorReason.message ? errorReason.message : `Something went wrong: l_ci_h_pse`;
    ...
    const onReceipt = function(receipt) {
      ...
      if ((isNaN(parsedIntStatus) && receipt.status !== true) || (!isNaN(parsedIntStatus) && parsedIntStatus !== 1)) {
    ...
    const onCatch = function(reason) {
      if (reason && reason.message && reason.message.includes('not mined within')) {
        oThis
          .getTransactionReceiptFromTrasactionHash(web3Provider, transactionHashRef)
          .then(function(getReceiptResponse) {
...
*/
    const asyncPerform = function() {
      return new Promise(function(onResolve, onReject) {
        ...
        web3Provider.eth.personal
          .unlockAccount(params.senderAddress, params.senderPassphrase)
          .then(function() {
            if (processReceipt) {
              web3Provider.eth
   ...
   *
   */
  validateCommonPayParams: async function(
    senderAddress,
    beneficiaryAddress,
    transferAmount,
  ...
  validatePayParams: async function(
    ...
    await oThis.validateCommonPayParams(
      senderAddress,
      beneficiaryAddress,
      transferAmount,
      commissionBeneficiaryAddress,
...
InstanceComposer.register(ContractInteractHelper, 'getContractInteractHelper', true);

module.exports = ContractInteractHelper;
