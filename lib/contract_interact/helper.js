...
const helper = {
      ...
      return new Promise(function (onResolve, onReject) {
        ...
        web3RpcProvider.eth.personal.unlockAccount(
          ...
          .catch(function(reason) {
            ...
            return Promise.resolve(responseHelper.error('errorCode', 'transaction failed'));
          });
  ...
  */
  isValidCurrency: function(currency){
    return (currency !== undefined && currency !== null && currency !== "" && currency !== 0);
  },
  ...
  validatePayParams: function(
    ...
    if (gasPrice === undefined || gasPrice === '' || gasPrice == 0  || gasPrice === null) {
      return responseHelper.error('l_ci_h_vpp_1', 'gas price is mandatory');
    }
    if (transferAmount < 0) {
      return responseHelper.error('l_ci_h_vpp_2', 'transfer amount cannot be negetive');
    }
    if (commissionAmount < 0) {
      return responseHelper.error('l_ci_h_vpp_3', 'Commission amount cannot be negetive');
    }
    ...
    if (transferAmount > 0 || (beneficiaryAddress !== undefined && beneficiaryAddress !== '')) {
      if (!helper.isAddressValid(beneficiaryAddress)) {
        return responseHelper.error('l_ci_h_vpp_3', 'address is invalid');
      }
    }
    if (commissionAmount > 0 || (commissionBeneficiaryAddress !== undefined && commissionBeneficiaryAddress !== '')) {
      if (!helper.isAddressValid(commissionBeneficiaryAddress)) {
        return responseHelper.error('l_ci_h_vpp_4', 'address is invalid');
      }
    }
    if (helper.isValidCurrency(currency)) {
      if (intendedPricePoint == undefined || intendedPricePoint === null || intendedPricePoint === "" || intendedPricePoint === 0) {
        return responseHelper.error('l_ci_h_vpp_5', 'intendedPricePoint is invalid for given currency');
      }
    }
  ...
  validateAirdropPayParams: function(
    ...
    const validationResponse = helper.validateAirdropPayParams(senderAddress,
      beneficiaryAddress,
      transferAmount,
      commissionBeneficiaryAddress,
      commissionAmount,
    ...
    if (!helper.isAddressValid(workerContractAddress)) {
      return responseHelper.error('l_ci_h_vapp_1', 'worker contract address is invalid');
    }

    if (!helper.isAddressValid(user)) {
      return responseHelper.error('l_ci_h_vapp_2', 'user address is invalid');
    }

    if (airdropAmount === undefined ) {
      return responseHelper.error('l_ci_h_vapp_3', 'airdrop amount should be atleast 0');
    }
