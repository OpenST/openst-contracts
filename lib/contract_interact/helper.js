...
const helper = {
  ...
  */
  ...
  validatePayParams: function(
    ...
    if (transferAmount < 0 || helper.isDecimal(transferAmount)) {
      return responseHelper.error('l_ci_h_vpp_2', 'transfer amount cannot be negative or floating value');
    }
    if (commissionAmount < 0 || helper.isDecimal(commissionAmount)) {
  ...
  validateAirdropPayParams: function(
    ...
    if (!airdropAmount || helper.isDecimal(airdropAmount) ) {
      return responseHelper.error('l_ci_h_vapp_3', "airdrop amount can't be blank. It should be atleast 0 and should not be decimal value");
    }
