...
BrandedToken.prototype = {
  ...
  creditBalance: async function (owner, bigAmount) {
    ...
    try {
      ...
      if (getBalanceOfResponse.isSuccess()) {
        ...
        logger.info('creditBalance called with params:', JSON.stringify({owner: owner,
      ...
      return Promise.resolve(getBalanceOfResponse);
    } catch(err) {
  ...
  debitBalance: async function (owner, bigAmount) {
    ...
    try {
      ...
      if (getBalanceOfResponse.isSuccess()) {
        ...
        logger.info('debitBalance called with params:', JSON.stringify({owner: owner,
          bigAmount: bigAmount.toString(10)}));
        await oThis.setBalanceInCache(owner, newBalance);
      } else {
  ...
  _beforeTransfer: function(address, amount) {
    ...
    logger.info('_beforeTransfer called with params:', JSON.stringify({address: address, amount: amount}));
  ...
  _afterTransferSuccess: function(address, amount) {
    ...
    logger.info('_afterTransferSuccess called with params:', JSON.stringify({address: address, amount: amount}));
  ...
  _afterTransferFail: function(address, amount) {
    ...
    logger.info('_afterTransferFail called with params:', JSON.stringify({address: address, amount: amount}));
