...
Pricer.prototype = {
  ...
  */
  brandedToken: async function () {
    ...
    try{
      ...
      return Promise.resolve(responseHelper.error('l_ci_p_bt_1', 'Something went wrong'));
    }
  },
  ...
  */
  acceptedMargins: async function (currency) {
    ...
    try {
      ...
      return Promise.resolve(responseHelper.error('l_ci_p_am_2', 'Something went wrong'));
    }
  ...
  */
  priceOracles: async function (currency) {
    ...
    try {
      ...
      return Promise.resolve(responseHelper.error('l_ci_p_po_2', 'Something went wrong'));
    }
  },
  ...
  */
  validateSetPriceOracleParams: function (senderAddress, currency, address, gasPrice) {
    if (!helper.isValidCurrency(currency, false)) return responseHelper.error('l_ci_p_spo_1', 'currency is mandatory');

    if (!gasPrice) return responseHelper.error('l_ci_p_spo_2', 'gas is mandatory');

    if (!basicHelper.isAddressValid(address)) return responseHelper.error('l_ci_p_spo_3', 'address is invalid');

    if (!basicHelper.isAddressValid(senderAddress)) return responseHelper.error('l_ci_p_spo_4', 'address is invalid');
  ...
  */
  validateUnsetPriceOracleParams: function (senderAddress, currency, gasPrice) {
    if (!helper.isValidCurrency(currency, false)) return responseHelper.error('l_ci_p_uspo_1', 'currency is mandatory');

    if (!gasPrice) return responseHelper.error('l_ci_p_uspo_2', 'gas is mandatory');

    if (!basicHelper.isAddressValid(senderAddress)) return responseHelper.error('l_ci_p_uspo_3', 'address is invalid');
  ...
  */
  validateSetAcceptedMarginParams: function (senderAddress, currency, acceptedMargin, gasPrice) {
    if (!helper.isValidCurrency(currency, false)) return responseHelper.error('l_ci_p_sam_1', 'currency is mandatory');

    if (!gasPrice) return responseHelper.error('l_ci_p_sam_2', 'gas price is mandatory');
    ...
    if (!basicHelper.isAddressValid(senderAddress)) return responseHelper.error('l_ci_p_sam_4', 'address is invalid');
  ...
  */
  verifyReceiptAndUpdateAcceptedMarginCache: async function (acceptedMargin, currency, receipt) {
    ...
    if (isReceiptValid) {
      ...
      return Promise.resolve(responseHelper.error('l_ci_p_sam_5', 'Invalid event in receipt'));
    }
  },
  ...
  */
  pay: async function (senderAddress, senderPassphrase, beneficiaryAddress, transferAmount, commissionBeneficiaryAddress,
    ...
    const successCallback = async function(receipt) {
      ...
      const actualAmountFromReceipt = oThis.transactionHelper.getActualAmountsFromReceipt(receipt, oThis.addressToNameMap, eventGlobalConstants.eventPayment());
  ...
  getPricePoint: async function (currency) {
    ...
    try {
      ...
      if (priceOraclesResponse.isFailure()) {
        return Promise.resolve(responseHelper.error('l_ci_p_gpp_4', 'Something went wrong'));
      }
